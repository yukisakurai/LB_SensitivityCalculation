    ...
    for j in range(0,n1):
        ...
        for k in range(0,num):
           ...
           #     print round(freq,2),", ",round(p_cmb*1.e24, 3),", ",round(p_hwp*1.e24, 3),", ",round(p_apt*1.e24, 3),", ",round(p_pm*1.e24, 3),", ",round(p_sm*1.e24, 3),", ",round(p_fil*1.e24, 3),", ",round(p_len*1.e24, 3),", ",round(p_opt*1.e24, 3),", ",round(p_20K*1.e24, 3)
            if freq==freqLFT[0][0]:
                 print round(freq,2),", ",p_hwp,", ",p_apt,", ",p_20K,", ",p_pm,", ",p_sm,", ",p_fil,", ",p_len,",",p_opt
                #print round(freq,2),", ",p_hwp
       ...
       # print round(freqLFT[i][j],2)," , ",round(Pcmb,5)
...
for i in range(0,m2):
    for j in range(0,n2):
        ...
        for k in range(0,num):
           ...
           # if freq==freqHFT[i][j]:
               # print round(freq,2),"hwp_emiss, ",hwp_emiss_HFT[i][j],"apt_emiss, ",apt_emiss,"lens1_emiss, ",pm_emiss,"lens2_eff, ",sm_emiss,"2Kfilter_emiss, ",fil_emiss,"lenslet_emiss, ",len_emiss
           ...
           # print round(freq,2),", ",p_cmb,", ",p_hwp,", ",p_apt,", ",p_pm,", ",p_sm,", ",p_fil,", ",p_len,", ",p_det
        ...
        print round(freqHFT[i][j],2)," , ",round(npixHFT[i][j],0)," , ",round(Popt,8)," , ",round(NEPopt,8)," , ",round(NEPth,3)," , ",round(NEPread,3)," , ",round(NEPint,2)," , ",round(NETdet,2)," , ",round(NETarrHFT[i][j],2)," , ",round(NETarrHFTmargin[i][j],2)
